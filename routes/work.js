...
router.post('/',async (req,res)=>{
    ...
    try{
        ...
        res.status(200).json(result(1,'Insert Work Success!'))
    }catch(error){
           res.status(500).json(result(0, error.message))
        }
})
...
router.put('/',async(req,res)=>{
    ...
    try{
        ...
        if(work.matchedCount>0){
            res.status(200).json(result(1,'Update Work Success!'))
        }else{
            res.status(200).json(result(0,'Update Work Failed!'))
        }
    }catch(error){
        res.status(500).json(result(0, error.message))
    }
...
router.delete('/:id',async(req,res)=>{
    try{
        ...
        if(work.deletedCount>0){
            res.status(200).json(result(1,'Delete Work Success!'))
        }else{
            res.status(200).json(result(0,'Delete Work Failed!'))
        }
    }catch(error){
        res.status(500).json(result(0, error.message))
    }
